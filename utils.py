...
def get_fundamentals(ticker):
    ...
    try:
        ...
        # --- 時価総額の単位調整（型エラーに強いバージョン） ---
        try:
            mkt_cap_float = float(mkt_cap) # 一旦むりやり数値に変換してみる
            if mkt_cap_float > 1_000_000_000_000:
                cap_str = f"{mkt_cap_float/1_000_000_000_000:.1f}兆"
            elif mkt_cap_float > 100_000_000:
                cap_str = f"{mkt_cap_float/100_000_000:.0f}億"
            else:
                cap_str = f"{mkt_cap_float:,.0f}"
        except:
            cap_str = "-"
            
        # --- PER, PBRの調整（型エラーに強いバージョン） ---
        try:
            per_str = f"{float(per):.2f}倍" if per != '-' and per is not None else "-"
        except:
            per_str = "-"

        try:
            pbr_str = f"{float(pbr):.2f}倍" if pbr != '-' and pbr is not None else "-"
        except:
            pbr_str = "-"
